...
app.put("/api/antibioticos/:codigo", async (req, res) => {
  try {
    ...
    // dispara alertas si queda bajo
    const lowRows = await dbQuery(
      "SELECT codigo, nombre, cantidad, stock_minimo FROM antibioticos WHERE cantidad <= stock_minimo ORDER BY nombre"
    );
    if (lowRows.rows.length) {
      try { await sendLowStockEmail(lowRows.rows); } catch {}
    }
...
app.post("/api/antibioticos/:codigo/restar", async (req, res) => {
  ...
  try {
    ...
    if (Number(item.cantidad) <= Number(item.stock_minimo)) {
      try { await sendLowStockEmail([item]); } catch {}
    }
...
app.post("/api/salidas", async (req, res) => {
  ...
  try {
    ...
    // Alertas: si alguno queda <= mínimo, email con todos los bajos
    const lowRows = await dbQuery(
      "SELECT codigo, nombre, cantidad, stock_minimo FROM antibioticos WHERE cantidad <= stock_minimo ORDER BY nombre"
    );
    if (lowRows.rows.length) {
